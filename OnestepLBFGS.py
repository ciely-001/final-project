...
def likelihood(Mat, Spect, Measures, x, device):
    ...
    y_bar_x = forward_mat_op(x_shaped, Mat.mass_attn, Spect.binned_spectrum, Measures.background, Measures.radon, False,
                             device)
...
def OneStepLBFGS(Mat, Spect, Measures, x_scout, beta, gamma, device):
    ...
    for m in range(Mat.n_mat):
        PSNR_OnestepLBFGS[m] = peak_signal_noise_ratio(x_mass_densities_np[0, m], x_lbfgs[0, m],
                                                       data_range=x_mass_densities_np[0, m].max())
        SSIM_OnestepLBFGS[m] = structural_similarity(x_lbfgs[0, m], x_mass_densities_np[0, m],
                                                     data_range=x_mass_densities_np[0, m].max(), gradient=False)

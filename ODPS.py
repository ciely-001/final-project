...
from forward_model import forward_mat_op
...
def likelihood(x, Mat, Spect, Measures, device):
    ...
    y_bar_x = forward_mat_op(x, Mat.mass_attn, Spect.binned_spectrum, Measures.background, Measures.radon, False,
                             device)
    ...
    exponent = Measures.radon1.forward(combined_mats.reshape(1, 150, Mat.img_size, Mat.img_size))

    att_term = torch.exp(-exponent)
    ...
    for idm in range(Mat.n_mat):
        ...
        inner = torch.sum(h_k_q.reshape(Spect.n_bin, 150, 1, 1) * att_term,
                          dim=1)  # sum over energies; this is the integral over E

        backprojected = Measures.radon1.backward1(inner * grad_phi)  # backproject integral * phi'
    ...
    for t in range(t_prime - 1, 0, -1):
        ...
        ###############
            ...
            with torch.enable_grad():
                ...
                x_proj = Measures.radon1.forward1(x_0_mat).to(device)
                ...
                grad = torch.autograd.grad(outputs=to_compute_grad, inputs=x_odps, retain_graph=True,
                                           grad_outputs=torch.ones_like(to_compute_grad))
        ...
        with torch.no_grad():
            ...
            x_odps = reverse_diffusion_from_x_pred(x_odps, x_0, t, ODPS.alpha[t], ODPS.alpha_bar[t], ODPS.sigma2[t],
                                                   Mat.img_size, ODPS.nn, device)
            ...
            for m in range(Mat.n_mat):
                PSNR_odps[t_prime - t, m] = peak_signal_noise_ratio(x_mass_density_np[0, m], x_odps_np[0, m],
                                                                    data_range=x_mass_density_np[0, m].max())
                SSIM_odps[t_prime - t, m] = structural_similarity(x_odps_np[0, m], x_mass_density_np[0, m],
                                                                  data_range=x_mass_density_np[0, m].max(),
                                                                  gradient=False)

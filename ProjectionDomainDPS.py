...
def ProjectionDomainDPS_method(Mat, Spect, Measures, DPS, x_scout, beta, t_prime, step, device):
    ...
    x_scout_proj = Measures.radon1.forward(x_scout)
    ...
    for t in range(t_prime - 1, 0, -1):
        ...
        with torch.enable_grad():
            ...
            grad = torch.autograd.grad(outputs=to_compute_grad, inputs=x, retain_graph=True,
        ...
        with torch.no_grad():
            ...
            x = reverse_diffusion_from_x_pred(x, x_0, t, DPS.alpha[t], DPS.alpha_bar[t], DPS.sigma2[t], Mat.img_size,
                                              DPS.nn, device)
            ...
            for m in range(Mat.n_mat):
                PSNR[t_prime - t, m] = peak_signal_noise_ratio(x_mass_density_np[0, m], x_np[0, m],
                                                               data_range=x_mass_density_np[0, m].max())
                SSIM[t_prime - t, m] = structural_similarity(x_np[0, m], x_mass_density_np[0, m],
                                                             data_range=x_mass_density_np[0, m].max(), gradient=False)

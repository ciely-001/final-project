...
def TDPS_method(Mat, Spect, Measures, TDPS, x_scout, t_prime, step, device):
    ...
    sino_approx_spectral = torch.log(
        torch.sum(Spect.binned_spectrum, dim=1)[:, None, None] / Measures.y) / Mat.pixel_size
    ...
    x_tdps = Mat.mass_attn_pseudo_spectral @ (x_scout.reshape(3, Mat.img_size ** 2) / Mat.pixel_size).float()
    ...
    for t in range(t_prime, 0, -1):
        ...
        radon_x_pred = Measures.radon1.forward1(x_0_LAC).to(device)
    ...
    x_tdps_mat = torch.linalg.lstsq(Mat.mass_attn_pseudo_spectral,
                                    (Mat.pixel_size * x_tdps_final[0, :].reshape(Spect.n_bin,
                                                                                 Mat.img_size ** 2)).float(),
                                    driver="gels").solution.reshape(3, Mat.img_size, Mat.img_size)[None, :, :, :]

...
from forward_model import radon_par
...
def PWLS(Mat, Spect, Measures, x_scout, n_iter, delta, beta_prior, device):
    ...
    D = Measures.radon1.backward(w * Measures.radon1.forward(torch.ones_like(x_pwls)))
    ...
    for n in range(n_iter):
        x_rec = x_pwls - Measures.radon1.backward(w * (Measures.radon1.forward(x_pwls) - sino_approx_spectral)) / D
        ...
        x_temp = torch.linalg.lstsq(Mat.mass_attn_pseudo_spectral,
                                    ...
                                    driver="gels").solution.reshape(3, Mat.img_size, Mat.img_size)[None, :, :, :]
        x_temp[x_temp < 0] = 0

        # Computing metrics :
        for k in range(Mat.n_mat):
            PSNR_pwls[n, k] = peak_signal_noise_ratio(x_mass_densities_np[0, k], x_temp[0, k].detach().cpu().numpy(),
                                                      data_range=x_mass_densities_np[0, k].max())
            SSIM_pwls[n, k] = structural_similarity(x_temp[0, k].detach().cpu().numpy(), x_mass_densities_np[0, k],
                                                    data_range=x_mass_densities_np[0, k].max(), gradient=False)
